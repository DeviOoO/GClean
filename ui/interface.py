...
class App:
    ...
    def otimizar(self):
        ...
        resultados = sysoptimize(self.progress_att)
    ...
    def atualizar_resumo_etapas(self, resultados):
        """Exibe o resumo de cada etapa de uma rotina (sucesso/falha + detalhe).

        Usado tanto pela Otimização de Sistema quanto pela Correção de Internet,
        já que ambas agora retornam list[Tuple[etapa, sucesso, detalhe]].
        """
        linhas = []
    ...
    def corrigir_net(self):
        ...
        resultados = netclean(self.progress_att)
        self.root.after(0, self.atualizar_resumo_etapas, resultados)
    ...
    def rotina_geral(self):
        ...
        resultados_net = netclean(lambda v: self.progress_att(0.33 + (v * 0.33)))

        # 3. Otimização do Sistema (Ocupa de 66% a 100% da barra)
        resultados_sys = sysoptimize(lambda v: self.progress_att(0.66 + (v * 0.34)))

        # Mostra o resumo combinado das duas rotinas baseadas em etapas
